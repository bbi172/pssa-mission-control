...
export async function POST(req: NextRequest) {
  ...
  for (const email of uniqueEmails) {
    ...
    if (!teacherId) {
      // Before sending a brand-new invite, check whether this email
      // already has a real account through another role (e.g. an admin
      // who is also becoming a teacher, or a returning teacher whose old
      // teacher row was deleted but who still has an admin role).
      const { data: existingAdminRow } = await supabaseAdmin
        .from('admins').select('user_id').eq('email', email).maybeSingle()
 
      let userId = existingAdminRow?.user_id
      ...
      const { data: newTeacher, error: teacherErr } = await supabaseAdmin
        .from('teachers')
        .insert({ user_id: userId, school_id: schoolId, name, email, grade_level: gradeLevel })
        .select('id').single()
      ...
      if (wasInvited) {
        ...
        summary.existingTeachers.push(`${email} (linked existing account — no new invite sent)`)
      }
    } else {
